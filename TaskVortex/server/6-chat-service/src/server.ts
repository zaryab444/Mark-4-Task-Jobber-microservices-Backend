...
const chatErrorHandler = (app: Application): void => {
  app.use((error: IErrorResponse, _req: Request, res: Response, next: NextFunction) => {
    console.log('error', `ChatService ${error.comingFrom}:`, error);
...
const startServer = async (app: Application): Promise<void> => {
  try {
    ...
    console.log('error', 'ChatService startServer() method error:', error);
  }
};
...
const createSocketIO = async (httpServer: http.Server): Promise<Server> => {
  const io: Server = new Server(httpServer, {
    cors: {
      origin: '*', //allow all enable connect of any client
      methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS']
    }
  });
  return io;
...
const startHttpServer = (httpServer: http.Server): void => {
  try {
    console.log(`Chat server has started with process id ${process.pid}`);
    httpServer.listen(SERVER_PORT, () => {
      console.log(`Chat server running on port ${SERVER_PORT}`);
    });
  } catch (error) {
    console.log('error', 'ChatService startHttpServer() method error:', error);
  }
};
