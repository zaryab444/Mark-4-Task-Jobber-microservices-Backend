...
const startServer = async (app: Application): Promise<void> => {
  try {
    ...
    console.log(`Review server has started with process id ${process.pid}`);
    httpServer.listen(SERVER_PORT, () => {
      console.log(`Review server running on port ${SERVER_PORT}`);
    });
  } catch (error) {
    console.log('error', 'ReviewService startServer() method error:', error);
  }
};
