...
const startServer = (app: Application): void => {
  try {
    ...
    console.log(`Gig server has started with process id ${process.pid}`);
    httpServer.listen(SERVER_PORT, () => {
      console.log(`Gig server running on port ${SERVER_PORT}`);
    });
  } catch (error) {
    console.log('error', 'GigService startServer() method error:', error);
  }
};
