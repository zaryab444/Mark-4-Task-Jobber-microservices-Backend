import { IAuth } from '@gateway/interfaces/auth.interface';
...
class AuthService {
    ...
    constructor() {
        this.axiosService = new AxiosService(`${config.AUTH_BASE_URL}/api/v1/auth`, 'auth');//this variable is opposite
        axiosAuthInstance = this.axiosService.axios; //this variable is used for protected routes like user login first
    }
    async getCurrentUser(): Promise<AxiosResponse> {
      ...
      async verifyEmail(token: string): Promise<AxiosResponse> {
        ...
        return response; // this response come from authservice when api gateway send the req.body data to authservice
      }
