...
const orderErrorHandler = (app: Application): void => {
  app.use((error: IErrorResponse, _req: Request, res: Response, next: NextFunction) => {
    console.log('error', `OrderService ${error.comingFrom}:`, error);
...
const startServer = async (app: Application): Promise<void> => {
  try {
    ...
    console.log('error', 'OrderService startServer() method error:', error);
  }
};
...
const startHttpServer = (httpServer: http.Server): void => {
  try {
    console.log(`Order server has started with process id ${process.pid}`);
    httpServer.listen(SERVER_PORT, () => {
      console.log(`Order server running on port ${SERVER_PORT}`);
    });
  } catch (error) {
    console.log('error', 'OrderService startHttpServer() method error:', error);
  }
};
