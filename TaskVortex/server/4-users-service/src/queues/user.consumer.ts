...
//this consumer message sent from the auth message when user creates an account
const consumeBuyerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const jobberQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false }); //this assert queue check if queue already exist or not
    await channel.bindQueue(jobberQueue.queue, exchangeName, routingKey); //this bind queue it will create a path between exchange queue using the routng key
    ...
    console.log('error', 'UsersService UserConsumer consumeBuyerDirectMessage() method error:', error);
  }
};


//this method listening from order service from create, approve and delete orders
const consumeSellerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    console.log('error', 'UsersService UserConsumer consumeSellerDirectMessage() method error:', error);
  }
};


//this method listen once buyer add a review for particular gig from review service
const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    await channel.assertExchange(exchangeName, 'fanout'); //for the fanout we dont need routing key 
    const jobberQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
    await channel.bindQueue(jobberQueue.queue, exchangeName, '');
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      const { type } = JSON.parse(msg!.content.toString());
      if (type === 'buyer-review') { //this if check coming from review service
        await updateSellerReview(JSON.parse(msg!.content.toString()));

        //this send message to gig service
        await publishDirectMessage(
          channel,
          'jobber-update-gig', //exchange name
          'update-gig', ///routing key
    ...
    console.log('error', 'UsersService UserConsumer consumeReviewFanoutMessages() method error:', error);
  }
};
