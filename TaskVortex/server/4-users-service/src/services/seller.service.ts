...
import { ISellerDocument } from '@users/interface/seller.interface';
import { IRatingTypes, IReviewMessageDetails } from '@users/interface/review.interface';
import { IOrderMessage } from '@users/interface/order.interface';
...
const updateTotalGigsCount = async (sellerId: string, count: number): Promise<void> => {
  await SellerModel.updateOne({ _id: sellerId }, { $inc: { totalGigs: count } }).exec(); //$inc is increment operator
};
